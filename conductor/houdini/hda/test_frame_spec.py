"""Test the frame_spec_ui.py module.
...
class BestClumpSizeTest(TestCase):
    ...
    def set_up(cls, clump_size, frame_iterable):
        ...
        frame_spec_ui.frame_set = Mock(return_value=set(frame_iterable))
        frame_spec_ui._update_sequence_stats = Mock()
        return (node, parm)

    def test_clump_size_should_remain_1(self):
        node, parm = self.set_up(clump_size=1, frame_iterable=xrange(1, 9, 1))
        frame_spec_ui.best_clump_size(node)
