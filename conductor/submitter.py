...
# Store the submitter instance so that we can reuse it later (re-open a
# closed window)
_ui_instance = None
...
class ConductorSubmitter(QtWidgets.QMainWindow):
    ...
    def runUiStandalone(cls):
        """Note that this UI class is to be run directly from a python shell,
        e.g. not within another software's context (such as maya/nuke)."""
    ...
    def runUi(cls, force_new=False):
        ...
        This is intended to be run within a software context
        such as Maya or Nuke (as opposed to a shell).  By
        default, this will show any existing submitter UI
        that had been closed prior by the user (as opposed
        to creating a new instance of the UI). Use the
    ...
    def getParentWindow(cls):
        """Return a QtWidget object that should act as the parent for the
        submitter window.

        This is oftentime the case when launching the
        submitter within another application such as nuke or
        maya.  In these cases, return the main QtWindow
    ...
    def createUI(self):
        ...
        self.ui_preemptible_chkbx.stateChanged.connect(
            self.savePreemptiblePref)
        self.ui_preemptible_chkbx.toggled.connect(
            self.ui_autoretry_wgt.setEnabled)
        ...
        # Hide find button (until maybe a later release TODO:(LWS) Come back to
        # this
        self.ui_find_pbtn.hide()

        # Connect context menu for job software treewidget
        self.ui_job_software_trwgt.setContextMenuPolicy(
            QtCore.Qt.CustomContextMenu)
        self.ui_job_software_trwgt.customContextMenuRequested.connect(
            self.openJobTreeMenu)

        # Connect context menu for available software treewidget
        self.ui_software_versions_trwgt.setContextMenuPolicy(
            QtCore.Qt.CustomContextMenu)
        self.ui_software_versions_trwgt.customContextMenuRequested.connect(
            self.openAvailableTreeMenu)

        self.ui_packages_splitter.setStretchFactor(0, 1)
        self.ui_packages_splitter.setStretchFactor(1, 2)

        # Hide the widget that holds advanced settings. TODO: need to come back
        # to this.
    ...
    def populateUi(self):
        ...
        This data should be global information that is not
    ...
    def applyDefaultSettings(self):
        ...
        # Set the default preemptible pref
        default_preempted_autoretry_count = CONFIG.get(
            'autoretry_policy', {}).get(
            "preempted", {}).get("max_retries")
        ...
        self.filterPackages(
            show_all_versions=self.ui_show_all_versions_chkbx.isChecked())
        ...
        # Populate the Job packages treewidget (this is file specific so goes
        # in this function, rather than self.populateUi
    ...
    def _setCustomRangeValidator(self):
        ...
        # the "range w option step" building block, e.g 100-100, or 100-100x3,
        # oor
        rx_range_w_step = r"(?:%s-%s(?:%s)?)+" % (rx_number,
                                                  rx_number, rx_step)
        # The final regex which uses a space and comma as a delimeter between
        # multiple frame strings
        rx_validation = "((%s|%s), +)+" % (rx_number, rx_range_w_step)
        self.frame_str_validator = QtGui.QRegExpValidator(
            QtCore.QRegExp(rx_validation), None)
    ...
    def addLoggingMenu(menu):
        ...
        # Create an action group (this allows for mutually exclusive checkboxes
        # (radio buttons)
        ...
        for level_name, level_value in sorted(
            ...
            # Set the action item to be checked if its log level matches the
            # current log level
    ...
    def addResetPreferencesMenu(self, menu):
        ...
        fle_func = functools.partial(
            self._resetPreferences,
            title,
            message,
    ...
    def _resetPreferences(self, title, message, filepath=None):
        ...
        If a filepath is given, then delete the preferenes
        for the given filepath. Otherwise delete the global
        preferences.
        """
        # Prompt the user
        yes, _ = pyside_utils.launch_yes_no_dialog(
            title, message, show_not_again_checkbox=False, parent=self)
    ...
    def populateCoresCmbx(self, **kwargs):
        ...
        previous_cores = self.ui_gpu_type_cmbx.itemText(self.ui_gpu_type_cmbx.currentIndex())
        ...
        for instance_type in sorted(instance_types, key=operator.itemgetter('cores')):
            c = instance_type['cores']
            if (instance_type.get('gpu') or {}).get('gpu_count', 0) > 0:
                cores_with_gpus.add(c)
            cores.add(c)
        new_cores_idx = 0
        for idx, c in enumerate(sorted(cores)):
            ...
            if c == previous_cores:
                new_cores_idx = idx
        self.ui_cores_cmbx.setCurrentIndex(new_cores_idx)
    ...
    def populateMemoryCmbx(self, **kwargs):
        ...
        '''
        instance_types = self.filterInstanceTypes(**kwargs)
        previous_memory = self.ui_gpu_type_cmbx.itemText(self.ui_gpu_type_cmbx.currentIndex())
        ...
        for idx, m in enumerate(sorted(mem)):
            ...
            if mem_str == previous_memory:
                new_memory_idx = idx
        self.ui_memory_cmbx.setCurrentIndex(new_memory_idx)
    ...
    def getSelectedMemory(self):
        ...
        if memory == '':
            return None
        return float(memory.split(' ')[0][:-len(self.memory_suffix)])
    ...
    def populateProjectCmbx(self):
        ...
        If any projects are specified in the config file,
        then use them. Otherwise query Conductor for all
        projects
        """
    ...
    def generateConductorArgs(self, data):
        """Return a dictionary which contains the necessary conductor argument
        names and their values.  This dictionary will ultimately be passed
        directly into a conductor Submit object when instantiating/calling it.

        Generally, the majority of the values that one would
        populate this dictionary with would be found by
        quering this UI, e.g.from the "frames" section of
        ui.
        """
        conductor_args = {}
    ...
    def getDockerImage(self):
        ...
        If there is a docker image in the config.yml file,
        then use it.
        """
    ...
    def getEnvironment(self):
        ...
        Merge any environment settings defined in the config
        with the environment of the packages that the user
        has selected.
        """
        #  Get any environment variable settings from config.yml
        config_environment = CONFIG.get("environment") or {}
        assert isinstance(config_environment,
                          dict), "Not a dictionary: %s" % config_environment
        selected_packages = self.getJobPackages()
        return package_utils.merge_package_environments(
            selected_packages, base_env=config_environment)

    def getSoftwarePackageIds(self):
        """Return the software packages that the submitted job will have access
        to when running.

        These packages are referred to by their ids. Merge
        the any packages that are defined in the config with
        those that are selected by the user.
        """
        config_package_ids = CONFIG.get("software_package_ids") or []
        assert isinstance(config_package_ids,
                          list), "Not a list: %s" % config_package_ids
        selected_package_ids = [package["package_id"]
    ...
    @staticmethod
    def getLocalUpload():
        """Return a bool indicating whether the uploading process should occur
        on this machine or whether it should get offloaded to the uploader
        daemon. If False, uploading will occur on the daemon.

        Simply return the value set in the config.yml.  In
        the future this option may be exposed in the UI
        """
    ...
    def launch_result_dialog(self, response_code, response):
        ...
        if response_code in SUCCESS_CODES_SUBMIT:
            ...
            message = (
                '<html><head/><body><p>Job submitted: '
                '<a href="%s"><span style=" text-decoration: underline; '
                'color:%s;">%s</span></a></p></body></html>') % (job_url,
                                                                 self.link_color,
                                                                 job_id)
            pyside_utils.launch_message_box(
                title, message, is_richtext=True, parent=self)
    ...
    def on_ui_submit_pbtn_clicked(self):
        ...
        try:
            ...
            if data:
                ...
                # Launch a dialog box what diesplays the results of the job
                # submission
    ...
    def runPostSubmission(self, data):
        ...
        The "data" argument contains the results of the main
        runConductorSubmission method, so that any results
        can be "inspected" and acted upon if desired.
        """
        return
    ...
    def getFrameRangeString(self):
        ...
        if self.ui_start_end_rdbtn.isChecked():
            return "%s-%sx%s" % (self.getStartFrame(),
                                 self.getEndFrame(), self.getStepFrame())
        else:
            return self.getCustomFrameString()

    def _validateCustomFramesText(self, text):
        """Validate that the given text conforms to a valid frame range
        expression If the text does not conform to the validator, colorize the
        lineedit so that it's apparent to the user."""
        if not text or self.frame_str_validator.validate(
                text, len(text))[0] == QtGui.QValidator.Invalid:
    ...
    def getUserPrefsGlobalWidgets(self):
        """Return a list of widget objects that are appropriate for restoring
        their state (values) from a user's preference file.

        Specifically, these widgets are those which a user's
        GLOBAL  preferences should be recorded/loaded for.
        These widgets are identified by a dynamic property
        that has been set on them via Qt Designer.
        """
    ...
    def getUserPrefsFileWidgets(self):
        ...
        Specifically, these widgets are those which a user's
        FILE-SPECIFIC should be recorded/loaded for. These
        widgets are identified by a dynamic property that
        has been set on them via Qt Designer.
        """
    ...
    def getSourceFilepath(self):
        ...
        This is  the currently opened maya/katana/nuke file,
        etc
        """
    ...
    def loadUserSettings(self):
        ...
        try:
            ...
            message = (
                "Unable to apply user settings. "
                "You may want to modify/delete settings here: %s" %
                settings_filepath)
    ...
    def saveUserSettings(self):
        ...
        These settings are recorded per source file
    ...
    def getEnforcedMd5s(self):
        ...
        # This is potentially TIME CONSUMING (if the files are large and
        # plenty).
        enforced_files = []
        ...
        for filepath in enforced_files:
            ...
            enforced_md5s[filepath] = common.generate_md5(
                filepath, base_64=True)
    ...
    def getDefaultScoutFrames(self):
        ...
        try:
            ...
            logger.warning(
                "Failed to load default scout frames\n%s",
                traceback.format_exc())

        # If there is failure or we can't derive the frame range, simply return
        # empty string
        return ""

    def getScoutFrames(self):
        """If the "Scout Job" checkbox is checked, promput the user to ender
        desired scout frames.

        If the user has submitted scout frames in the past,
        then prepopulate those frames in the dialog box.
        Otherwise generate default scout frames
        """

        # If the button that was pressed was the submit_job button (not the
        # submit scout job button)
        if self.getScoutJobCheckbox():

            source_filepath = self.getSourceFilepath()

            # Retrieve any scout frames from the user's prefs, or use defaults
            default_scout_frames = self.prefs.getFileScoutFrames(
                source_filepath) or self.getDefaultScoutFrames()

            # Launch the scout frames input dialog
            ok, scout_frames = self.launchScoutFramesDialog(
    ...
    def getAutoretryPolicy(self):
        """Construct/return an autoretry policy if the user as selected to use
        preemptible instances."""
        # Only return an autoretry policy if the user has specified preemptible
        # instances
    ...
    def launchScoutFramesDialog(self, default_scout_frames):
        ...
        Prepopulate the lineedit field with any scout frames
        that have been submitted previous for the current
        file (read from preferences).
        """
        lineedit_tooltip = (
            "Specify desired scout frames:\n"
        ...
        # Trigger the validator immediately so that an empty lineedit will be
        # validated against
    ...
    @common.ExceptionLogger(
        "Failed to save Conductor user preferences. You may want to reset your preferences from the options menu")
    def saveScoutJobPref(self, is_checked):
        ...
        return self.prefs.setFileWidgetPref(
            filepath, widget_name, bool(is_checked))

    @common.ExceptionLogger(
        "Failed to save Conductor user preferences. You may want to reset your preferences from the options menu")
    def savePreemptiblePref(self, is_checked):
        """Save the "Preemptible" checkbox user preference for the open
        file."""
        filepath = self.getSourceFilepath()
        widget_name = self.sender().objectName()
        return self.prefs.setFileWidgetPref(
    ...
    def populateJobPackages(self, auto=False):
        ...
        source_filepath = self.getSourceFilepath()
        software_package_ids = self.prefs.getJobPackagesIds(
            source_filepath) or []
        ...
        # If no packages have been found in prefs or config, then autodetect
        # packages
            ...
            package_strs = [
                ...
                (p["product"],
                 p["version"],
                    "(for %s: %s)" %
                    (p["host_product"],
                     p["host_version"]) if p.get("host_product") else "") for p in unmatched_software]
    ...
    def saveJobPackagesToPrefs(self):
        package_ids = []
        for item in pyside_utils.get_top_level_items(
                self.ui_job_software_trwgt):
    ...
    def populateSoftwareVersionsTrWgt(self):
        """Populate the QTreeWidget with all of the available software
        packages.
        ...
        # Create a dictioary of the software packages so that they can be
        # retried by ID
        self.software_packages = dict(
            [(package["package_id"], package) for package in software_packages])
        ...
        for host_package in host_packages:
            ...
            for plugin_id in host_package["plugins"]:
                ...
                if not plugin_package:
                    logger.warning(
                        "Not able to find expected plugin. Id: %s", plugin_id)
                    continue

                # PLUGIN PRODUCT GROUP ####
                plugin_product_name = plugin_package["product"]
                plugin_parent_product_item = plugin_parent_items.get(
                    plugin_product_name)
                if not plugin_parent_product_item:
                    plugin_parent_product_item = QtWidgets.QTreeWidgetItem(
                        [plugin_product_name, ""])
                    plugin_parent_product_item.setFlags(
                ...
                plugin_package_item = self.createPackageTreeWidgetItem(
                    plugin_package)
                plugin_parent_product_item.addChild(plugin_package_item)

        # Sort the row by the version column
        software_column_idx = 1
        self.ui_software_versions_trwgt.sortByColumn(
            software_column_idx, QtCore.Qt.AscendingOrder)
...
#                     self.ui_software_versions_trwgt.setItemHidden(host_package_item, False)
    ...
    def getTreeItemPackages(self):
        tree_item_packages = {}
        for tree_item in pyside_utils.get_all_tree_items(
                self.ui_software_versions_trwgt):
            if hasattr(tree_item, "package_id"):
                tree_item_packages[tree_item.package_id] = self.get_package_by_id(
                    tree_item.package_id)
...
#         return maching_packages[0]
    ...
    def getMatchingPackage(self, software_info, packages, strict=False):
        matching_packages = package_utils.get_matching_packages(
            software_info, packages)
    ...
    def getSelectedAvailablePackages(self):
        """Return the package dictionaries for each QTreeItem that is selected
        by the user."""
        selected_packages = []
    ...
    def openAvailableTreeMenu(self, position):
        ...
        menu.exec_(
            self.ui_software_versions_trwgt.viewport().mapToGlobal(position))

    def _makeAvalailableSoftwareContextMenu(self, selected_item):
        """Remove selected items Add selected items Show items in (other)
        tree."""
        menu = QtWidgets.QMenu()

        # "check selected" menu item
        menu.addAction(
            "Add selected packages to Job",
            self._availableAddSelectedItems)
        return menu
    ...
    def _selectAvailablePackages(self, packages, clear=True):
        ...
        for tree_item in pyside_utils.get_all_tree_items(
            ...
            for package in packages:
                if hasattr(
                        tree_item,
                        "package_id") and tree_item.package_id == package["package_id"]:
    ...
    def validateJobPackages(self):
        """Validate that all packages that have been added to the job
        are...valid.

        Return True if so, otherwise launch a dialog box to
        the user.
        """
        ...
        duplicate_ids = set(
            [x for x in job_package_ids if job_package_ids.count(x) > 1])
        duplicate_packages = [self.get_package_by_id(
            package_id) for package_id in duplicate_ids]

        if duplicate_packages:
            title = "Duplicate software packages!"
            dupe_package_strs = sorted(
                ["- %s" % self._constructJobPackageStr(p) for p in duplicate_packages])
            msg = (
                "Duplicate software packages have been specified for the Job!\n\n"
        ...
        if duplicate_product_strs:
            ...
            msg = (
                "Multiple software packages of the same product have been specified for the Job!\n\n"
                "Please go the \"Job Software\" tab and ensure that only one "
        ...
        for package in self.getJobPackages():
            ...
            msg = (
                "No %s software package have been specified for the Job!\n\n"
                "Please go the \"Job Software\" tab and select a one (potentially %s") % (self.product,
                                                                                          host_info["version"])
            pyside_utils.launch_error_box(title, msg, parent=self)
            self.ui_tabwgt.setCurrentIndex(self._job_software_tab_idx)
            return False
...
class SubmitterPrefs(pyside_utils.UiFilePrefs):
    ...
    """A class for interfacing with User preferences for the submitter UI. This
    subclasses adds functionality that is specific to the Conductor submitter.
    ...
    """
    # PREFERENCE NAMES (these the names that are used/written to the
    # preference file (non widget)
    PREF_SCOUTFRAMES_STR = "scoutframes_str"
    PREF_JOB_PACKAGES_IDS = "job_packages_ids"

    @common.ExceptionLogger(
        "Failed to load Conductor user preferences. You may want to reset your preferences from the options menu")
    def loadSubmitterUserPrefs(self, source_filepath):
        ...
        First load the global preferences, then load the
        file-specific prefs which will override and of the
        global prefs
        """
...
class TaskFramesGenerator(object):
    ...
    differently for each task (e.g. so that each task renders different frames,
    etc).
    ...
    def __init__(self, frames, chunk_size=1, uniform_chunk_step=True):
        ...
        # copy the original frame list. This will track which frames have not
        # been dispensed yet
    ...
    def next(self):
        ...
        # to be allocated)
        next_task_frames = self._next(
            self._chunk_size, self._uniform_chunk_step)
        if next_task_frames:
            # Return the command for the task, and the accompanying frames
            # list.
    ...
    def _next(self, chunk_size, uniform_chunk_step):
        """Construct and return the command to be executed for a task. This
        command is specific per product.  The returned command should be fully
        resolved and ready to be executed by the task. In other words, resolve
        all arguments for the command, such as the frames(s) to be rendered
        (taking into consideration steps and chunks, etc).

        Call this method repeatedly to dispense a new task
        command (until all frames have been
        dispensed/allocated).
        """
        ...
        if task_frames:
            ...
            # Validate that the there is only 1 step between the frames in the
            # chunk
            assert len(steps) == 1, "More than one step in frames chunk. Frames chunk: %s.  Steps found: %s" % (
                task_frames, steps)
    ...
    def get_next_frames_chunk(self, chunk_size, uniform_chunk_step=True):
        ...
        for _ in range(chunk_size):
            ...
            # If all frames have been dispensed, return the chuck (which could
            # be empty)
            ...
            # Or if the frame follows the same step as the rest of the chunk
            # frames (it's uniform)
            if not uniform_chunk_step or not task_frames or len(
                    set(self.derive_steps(task_frames + (next_frame,)))) == 1:
                task_frames += (next_frame,)

                # Pop a frame off the undispensed listnext_frames_chunk
                self._undispensed_frames.pop(0)

            # Otherwise break the loop (and return whatever the chunk is at
            # this point)
            else:
                break
    ...
    def derive_steps(cls, frames):
        """Inspect the list of frames and derive what the "step" is between
        them. If more than one step is found, return the lowest and highest
        steps.
        ...
        """
        # make a copy of the frames list, and then sort it (it should be sorted
