...
def collect_dependencies(node_attrs):
            ...
            for node_attr in node_attrs:
                ...
                if cmds.objExists(plug_name):
                    ...
                    if node_type == "aiStandIn" and path:
                        ...
                        # file in an .ass sequeuence will have the same file dependencies, so don't bother reading every
    ...
    # Note that we intentially maintain an explicit list of .ass files (rather than simply searching
    # through all of our dependencies thus far for a .ass extension) because there may some ass files
    # that are dependecies, but are not necessary (or redundant) to load/parse (i.e. in the case of
    # ass sequences.
    if ass_filepaths:
...
def scrape_ass_files(ass_filepaths, node_attrs, plugin_paths=()):
    ...
    plugin_paths: tuple of strings. This may be necessary when running outside of maya/interactive.
        so that noes for arnold plugins (yeti, etc) can be properly loaded/used.
