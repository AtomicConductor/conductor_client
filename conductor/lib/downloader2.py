...
class Downloader(object):
  ...
  7.
    ...
    # a messaging object to indicate when 
    _job_downloads_finished = multiprocessing.Array('c', 'false')
    ...
    def download_jobs(cls, job_ids, task_ids=None, thread_count=None, output_dir=None):
        ...
        while not common.SIGINT_EXIT and downloader._job_downloads_finished.value != "true":
            time.sleep(0.1)
    ...
    def queue_target2(self, pending_queue, downloading_queue, jids, tids=None):
        '''
        '''
        for jid in jids:
            try:
        ...
        self._job_downloads_finished.value == "true"
    ...
    def enqueue_job_downloads(self, jid, tids=None, page_size=200):
        ...
        start_cursor = None
        while not common.SIGINT_EXIT:
            logger.info("Fetching job %s downloads...", jid)
