...
The Conductor submitter for Maya allows you to ship renders to Conductor's cloud from a familiar interface within Maya. It is implemented as a custom class that exists inside the project. The class name is ConductorJob.
...
## Register the plugin
To register the submitter, set the provided startup script in the prefereces panel. It will take effect the next time you start Maya.
...
You'll notice the project is **- not set -** and the pulldown menu is empty. This is because the submitter has not yet connected to your account. 
...
You'll notice the **Frames Info** attribute has updated so you can see which frames will be submitted, and how many will be scouted first. 

![frames][frames]

* If you have some idea of the machine specification needed for your images, choose it in the **Instance Type** drop-down menu.
* Check that your version of Maya appears in the **Packages** attribute. If it hasn't been detected, you'll need to open the **Choose Packages** panel and choose a suitable version.
