...
import conductor.houdini.lib.sequence.progressions as prog
...
class ProgressionsTest(unittest.TestCase):
    ...
    def test_max_size_one(self):
        result = prog.create([2, 4, 6, 8, 10], max_size=1)
        self.assertEqual(len(result), 5)

    def test_max_2(self):
        result = prog.create([2, 4, 6, 8, 10], max_size=2)
        self.assertEqual(len(result), 3)

    def test_max_16(self):
        result = prog.create(xrange(2, 197, 3), max_size=16)
